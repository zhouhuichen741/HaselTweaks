...
- **Changed:** Increased the delay between items being added and the check for daily quest with recipe step from 100ms to 500ms and added an optional second check after 1 second.
...
Replaced [Intel® Intelligent Storage Acceleration Library (ISA-L)](https://github.com/intel/isa-l) with [libdeflate](https://github.com/ebiggers/libdeflate), which is even faster. :)
...
- **Maybe fixed:** An error "Unsupported image format. Expected DXGI_FORMAT_B8G8R8A8_UNORM, got DXGI_FORMAT_B8G8R8A8_TYPELESS." when saving presets. I just allowed the TYPELESS format too, even though I don't know how it can be TYPELESS. Hope it works!
...
- **Changed:** The plugin now sets up ClientStructs and the services asynchronously for improved startup performance. After that, the tweaks are enabled on the main thread as before.
- **Changed:** The link to my Ko-Fi page was replaced with a link to my [Github Sponsors](https://github.com/sponsors/Haselnussbomber) page. Thank you all for your support so far! <3
...
Not so sure about this yet, but it's a start.
...
- **Fixed:** The windows draw condition was incorrectly inverted.
